...
/* Images */
import christmasHouse from '@/assets/images/christmas-house.png'
import christmasTree from '@/assets/images/christmas-tree.png'
import christmasWreath from '@/assets/images/christmas-wreath.png'
import gingerbreadMan from '@/assets/images/gingerbread-man.png'
import santaClaus from '@/assets/images/santa-claus.png'
import stocking from '@/assets/images/stocking.png'
import gift from '@/assets/images/gift.png'

import merryChristmas from '@/assets/lottie/merry_christmas.json'

/* Sound */

const winSoundFile = 'src/assets/sounds/win.wav'
const winSound = new Audio(winSoundFile)

const hasCompleted = useLocalStorage('hasCompleted', false)
...
const scrollToGame = () => {
  if (appContainer.value) {
    const currentScroll = appContainer.value.scrollLeft
    const sectionWidth = appContainer.value.clientWidth
    appContainer.value.scrollTo({ left: currentScroll + sectionWidth, behavior: 'smooth' })
  }
}

const handleScroll = (event: any) => {
  // Ensure appContainer is defined
  if (!appContainer.value) return

  // Get the width of a section
  const sectionWidth = appContainer.value.clientWidth

  // Determine the scroll direction and calculate the new scroll position
  const newScrollPosition =
    event.deltaY > 0
      ? appContainer.value.scrollLeft + sectionWidth
      : appContainer.value.scrollLeft - sectionWidth
 ...
 * Snow Flakes
 */
const snowFlakes = 300

/**
 * Board Game
 */
const isOpened = ref(false)
const toggleCard = () => (isOpened.value = !isOpened.value)

const drawings = [christmasTree, christmasWreath, gingerbreadMan, santaClaus, stocking, gift]
const contents = [...drawings, ...drawings].sort(() => Math.random() - 0.5)
const cards = ref(contents.map((content) => ({ content, flipped: false })))
const matchedCards = ref<string[]>([])
const flippedCards = ref<number[]>([])

const checkForMatch = () => {
  setTimeout(() => {
    const firstCard = cards.value[flippedCards.value[0]]
    const secondCard = cards.value[flippedCards.value[1]]

    if (firstCard.content === secondCard.content) {
      matchedCards.value.push(firstCard.content)
    } else {
      cards.value[flippedCards.value[0]].flipped = false
      cards.value[flippedCards.value[1]].flipped = false
    }
...
const flipCard = (idx: number) => {
  // Check if the card is already matched or currently flipped
  if (matchedCards.value.includes(cards.value[idx].content)) return

  if (flippedCards.value.length < 2 && !cards.value[idx].flipped) {
    cards.value[idx].flipped = true
    flippedCards.value.push(idx)

    if (flippedCards.value.length === 2) checkForMatch()
    return
  }

  if (flippedCards.value.length < 2 && cards.value[idx].flipped) {
    cards.value[idx].flipped = false
    flippedCards.value.pop()
  }
}

const isGameCompleted = computed(() => matchedCards.value.length === cards.value.length / 2)

whenever(isGameCompleted, () => {
...
onMounted(() => {
  if (window.innerWidth < 768)
    alert('This app is best used on a desktop or laptop. Please switch to a larger screen to play.')
})
</script>
...
<template>
  <div
    ref="appContainer"
    class="flex flex-row overflow-x-auto snap-x snap-mandatory h-screen overflow-y-hidden whitespace-nowrap"
    @wheel="handleScroll"
  >
    <!-- Section 1: Christmas House -->
    <section
      class="snap-center w-screen h-screen flex-grow-0 flex-shrink-0 bg-center bg-no-repeat bg-cover relative"
      :style="`background-image: url(${christmasHouse})`"
    >
      <!-- Show Snow Drops -->
      <div class="absolute top-0 left-0 w-full h-full overflow-hidden">
        <div class="snow" v-for="snow of snowFlakes" :key="snow"></div>
      </div>
      ...
      <div class="w-full h-full flex flex-col justify-center items-center">
        ...
        >
          <Vue3Lottie
            ...
            :loop="false"
          ></Vue3Lottie>
          <p class="w-full relative -mt-8 font-bold text-secondary px-4 leading-5">
            Thank you for your friendship and support throughout the year! Your presence has been a
            gift in itself, making the tough times easier and the good times even brighter. I hope
            this holiday season brings you everything you wish for.
          </p>
        </div>
      </div>

      <!-- Make User Click to Scroll Right -->
      <button
        class="absolute right-2 top-1/2 -translate-y-[50%] text-white text-6xl animate-pulse"
        @click="scrollToGame"
      >
        &#9654;
      </button>
    ...
    >
      <!-- After Win: Christmas Card -->
      <section
        class="absolute top-0 left-0 transition-all z-50 w-screen h-screen"
        style="background: radial-gradient(at top, #12313b, #000822)"
        :style="{
          transform: hasCompleted ? 'translateY(0vh)' : 'translateY(-100vh)',
          opacity: hasCompleted ? 1 : 0
        }"
      >
        <!-- Confetti -->
        <div v-for="n in 200" :key="n" class="confetti-piece" :style="getConfettiStyle(n)"></div>
        <!-- Christmas Card -->
        <div class="w-full h-full flex justify-center items-center flex-col gap-6 relative">
          <div class="christmas-card" :class="{ 'is-opened': isOpened }" @click="toggleCard">
            <div class="card-page cart-page-front">
              <div class="card-page cart-page-outside"></div>
              <div class="card-page cart-page-inside">
                <span class="merry-christmas" :class="{ 'is-hidden': isOpened }">
                  <svg
                    version="1.1"
                    id="Layer_2"
            ...
            <div class="card-page cart-page-bottom">
              <div class="w-full">
                <p class="text-secondary font-bold text-base">
                  ...
                  here is a Gift Card for Pinkoi!
                </p>
                <p class="mt-1 text-primary font-bold text-base">{{ giftCard }}</p>
              </div>
            </div>
          </div>
        </div>
        <!-- Click Instruction -->
        <span
          class="click-icon translate-y-[-100%]"
          :class="{
            'opacity-0': isOpened
          }"
        >
      ...
      <div class="w-full h-full flex justify-center items-center flex-col gap-6">
        <!-- Game Title -->
        <h1 class="text-white font-extrabold text-2xl">Here is a Christmas Game with a Prize 😊</h1>
        <!-- Game Board Section -->
        <section
          class="bg-secondary w-[450px] h-[350px] rounded-md flex flex-wrap justify-evenly items-center p-3 gap-2"
        >
          <!-- Cards -->
          <div
          ...
          >
            <div
              class="card-inner rounded-md border-2 border-accent"
              :class="{ 'is-flipped': card.flipped }"
            >
              <div class="bg-primary rounded-md card-front"></div>
              <div
                class="card-back rounded-md bg-cover bg-center bg-no-repeat"
                :style="`background-image: url(${card.content})`"
              ></div>
            </div>
          </div>
        </section>
